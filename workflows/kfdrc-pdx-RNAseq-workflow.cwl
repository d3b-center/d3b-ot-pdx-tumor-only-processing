...
steps:
  ...
  bam2fastq:
    run: ../kf-rnaseq-workflow/tools/samtools_fastq.cwl
    in:
  ...
  cutadapt:
    run: ../kf-rnaseq-workflow/tools/cutadapter.cwl
    in:
  ...
  xenome_classify:
    ...
    in:
      ...
      fastq_reads:
        source: [cutadapt/trimmedReadsR1, cutadapt/trimmedReadsR2]
        valueFrom: "${ return self; } "
      output_basename: sample_name
    out:
      [ graft_fastqs, host_fastqs, output_stats ]
  ...
  star:
    run: ../kf-rnaseq-workflow/tools/star_align.cwl
    in:
  ...
  samtools_sort:
    run: ../kf-rnaseq-workflow/tools/samtools_sort.cwl
    in:
  ...
  strand_parse:
    run: ../kf-rnaseq-workflow/tools/expression_parse_strand_param.cwl
    in:
      wf_strand_param: wf_strand_param
    out: [rsem_std, kallisto_std, rnaseqc_std, arriba_std]

  star_fusion:
    run: ../kf-rnaseq-workflow/tools/star_fusion.cwl
    in:
  ...
  arriba_fusion:
    run: ../kf-rnaseq-workflow/tools/arriba_fusion.cwl
    in:
  ...
  rsem:
    run: ../kf-rnaseq-workflow/tools/rsem_calc_expression.cwl
    in:
  ...
  rna_seqc:
    run: ../kf-rnaseq-workflow/tools/RNAseQC.cwl
    in:
  ...
  supplemental:
    run: ../kf-rnaseq-workflow/tools/supplemental_tar_gz.cwl
    in:
      outFileNamePrefix: output_basename
  ...
  kallisto:
    run: ../kf-rnaseq-workflow/tools/kallisto_calc_expression.cwl
    in:
  ...
  annofuse:
    run: ../kf-rnaseq-workflow/workflow/kfdrc_annoFuse_wf.cwl
    in:
...
- STAR
sbg:links:
- id: 'https://github.com/kids-first/kf-rnaseq-workflow/releases/tag/v2.4.1'
